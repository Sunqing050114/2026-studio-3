...
/**
 * Component used to store information related to combat such as health, attack, armor and
 * status effects. Any entities which engage it combat should have an instance of this class
 * registered. This class can be extended for more specific combat needs.
 */
public class CombatStatsComponent extends Component {
    ...
    /**
     * Applies a status effect to this entity. If an effect with the same id is already active,
     * it is overwritten by the new one (design choice: overwrite, not stack. To be confirmed
     * with Team 5/6 if card design expects stacking behaviour instead).
     *
    ...
    /**
     * Ticks down the duration of all active status effects by one and removes any that have
     * expired. This method's internal logic (tick/expire/cleanup) is self-contained.
     * IMPORTANT - external dependency: this method must be called exactly once per turn for
     * durations to mean "number of turns". WHEN it gets called is not yet wired up - it depends
     * on Team 3's turn/battle-sequence event, which is not confirmed yet.
     */
    ...
    /**
     * PLACEHOLDER / NOT FINAL: applies incoming-damage modifiers from active status effects to a
     * raw damage amount, before armor absorption.
     * This currently only demonstrates the mechanism using a single hard-coded example effect
     * ("vulnerable", +50% incoming damage) so that the armor+status-effect pipeline in
     * takeDamage() can be built and tested end-to-end. The actual set of supported status effect
     * types, and how each one's effectValue should be interpreted (percentage vs flat amount,
     * multiplicative vs additive, etc.), still needs to be confirmed with Team 5 (card effects)
     * and Team 6 (card data model). This method WILL need to be
