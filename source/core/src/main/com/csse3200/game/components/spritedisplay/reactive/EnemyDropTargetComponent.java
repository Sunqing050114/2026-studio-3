...
/**
 * Makes any entity a valid drop target for card drag-and-drop, purely from its world-space
 * position/scale. No dependency on the entity having any UI/display component — the drop hitbox is
 * computed by projecting the entity's world position into stage coordinates every frame.
 *
 * <p>This component is intentionally game-agnostic: it doesn't know what "damage" or "heal" or
 * "updateHealth" mean. It just unpacks whatever {@link TriggerPayload} the dragged card carried and
 * fires that event on the drop target, with whatever args the card was configured with (see Card /
 * CardService / ClickableRecord.args()). Whatever component cares about that event (e.g.
 * PlayerActions) listens for it and decides what it means. UI feedback ("Strike played") is handled
 * separately by DragNDrop, on the card's own entity — see DragNDrop.dragStop().
 *
 * <p>DEBUG_VISUAL draws a translucent red rectangle over the actual hitbox so you can visually
 * confirm the drop zone lines up with where you're dragging. Leave off unless you're actively
 * debugging alignment — it allocates a Texture per instance.
 */
public class EnemyDropTargetComponent extends Component {
  ...
  /**
   * Dispatches the payload's trigger with its args, matching EventHandler's supported arities
   * (0–3). Add a case here if a card ever needs more than 3 arguments — though consider bundling
